...
void Ac3TimeStampCalc::SetParameters(uint32 aFreq, uint32 aSamples)
{
    if (0 != aFreq)
    {
        iSamplingFreq = aFreq;
...
void Ac3TimeStampCalc::SetFromInputTimestamp(uint32 aValue)
{
    iCurrentTs = aValue;
    iCurrentSamples = 0;
}
...
void Ac3TimeStampCalc::UpdateTimestamp(uint32 aValue)
{
    iCurrentSamples += aValue;
}

//Convert current samples into the output timestamp
uint32 Ac3TimeStampCalc::GetConvertedTs()
{
...
uint32 Ac3TimeStampCalc::GetCurrentTimestamp()
{
    uint32 Value = iCurrentTs;
...
uint32 Ac3TimeStampCalc::GetFrameDuration()
{
    uint32 Value = 0;
