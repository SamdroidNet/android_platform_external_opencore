...
void EvrcTimeStampCalc::SetParameters(uint32 aFreq, uint32 aSamples)
{
    if (0 != aFreq)
    {
        iSamplingFreq = aFreq;
...
void EvrcTimeStampCalc::SetFromInputTimestamp(uint32 aValue)
{
    iCurrentTs = aValue;
    iCurrentSamples = 0;
}
...
void EvrcTimeStampCalc::UpdateTimestamp(uint32 aValue)
{
    iCurrentSamples += aValue;
}

//Convert current samples into the output timestamp
uint32 EvrcTimeStampCalc::GetConvertedTs()
{
...
uint32 EvrcTimeStampCalc::GetCurrentTimestamp()
{
    uint32 Value = iCurrentTs;
...
uint32 EvrcTimeStampCalc::GetFrameDuration()
{
    uint32 Value = 0;
