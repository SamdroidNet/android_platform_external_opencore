 ...
 * -------------------------------------------------------------------
 */
#ifndef PVMF_EVRCFFPARSER_NODE_H_INCLUDED
#define PVMF_EVRCFFPARSER_NODE_H_INCLUDED
...
#ifndef PVMF_EVRCFFPARSER_PORT_H_INCLUDED
...
#define PVMF_EVRC_PARSER_NODE_NEW(auditCB,T,params,ptr)\
{\
	ptr = OSCL_NEW(T,params);\
}

#define PVMF_EVRC_PARSER_NODE_DELETE(auditCB,T,ptr)\
{\
	OSCL_DELETE(ptr);\
}

#define PVMF_EVRC_PARSER_NODE_TEMPLATED_DELETE(auditCB, T, Tsimple, ptr)\
{\
	OSCL_DELETE(ptr);\
}

#define PV_EVRC_PARSER_NODE_ARRAY_NEW(auditCB, T, count, ptr)\
{\
	ptr = OSCL_ARRAY_NEW(T, count);\
}

#define PV_EVRC_PARSER_NODE_ARRAY_DELETE(auditCB, ptr)\
{\
	OSCL_ARRAY_DELETE(ptr);\
}


typedef PVMFGenericNodeCommand<OsclMemAllocator> PVMFEVRCFFNodeCommandBase;

/* Default vector reserve size */
#define PVMF_EVRC_PARSER_NODE_VECTOR_RESERVE 10
/* Starting value for command IDs  */
#define PVMF_EVRC_PARSER_NODE_COMMAND_ID_START 9000

enum TPVMFEVRCParserNodeCommand
{
    PVMF_EVRC_PARSER_NODE_QUERYUUID,
    PVMF_EVRC_PARSER_NODE_QUERYINTERFACE,
    PVMF_EVRC_PARSER_NODE_REQUESTPORT,
    PVMF_EVRC_PARSER_NODE_RELEASEPORT,
    PVMF_EVRC_PARSER_NODE_INIT,
    PVMF_EVRC_PARSER_NODE_PREPARE,
    PVMF_EVRC_PARSER_NODE_START,
    PVMF_EVRC_PARSER_NODE_STOP,
    PVMF_EVRC_PARSER_NODE_FLUSH,
    PVMF_EVRC_PARSER_NODE_PAUSE,
    PVMF_EVRC_PARSER_NODE_RESET,
    PVMF_EVRC_PARSER_NODE_CANCELALLCOMMANDS,
    PVMF_EVRC_PARSER_NODE_CANCELCOMMAND,
    PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_POSITION,
    PVMF_EVRC_PARSER_NODE_QUERY_DATASOURCE_POSITION,
    PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_RATE,
    PVMF_EVRC_PARSER_NODE_GETNODEMETADATAKEYS,
    PVMF_EVRC_PARSER_NODE_GETNODEMETADATAVALUES,
    PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_DIRECTION,
    PVMF_EVRC_PARSER_NODE_GET_LICENSE_W,
    PVMF_EVRC_PARSER_NODE_GET_LICENSE,
    PVMF_EVRC_PARSER_NODE_CMD_CANCEL_GET_LICENSE,
    PVMF_EVRC_PARSER_NODE_COMMAND_LAST
};

#define PVMF_EVRC_PARSER_NODE_MAX_CPM_METADATA_KEYS 256

class PVMFEVRCFFNodeCommand : public PVMFEVRCFFNodeCommandBase
{
    public:
        ...
        {
            PVMFEVRCFFNodeCommandBase::Construct(s, cmd, aContext);
        ...
        void Destroy()
        {
            PVMFGenericNodeCommand<PVMFEVRCParserNodeAllocator>::Destroy();
            switch (iCmd)
            {
                case PVMF_EVRC_PARSER_NODE_GETNODEMETADATAKEYS:
                    if (iParam4)
                    {
                        ...
                        Oscl_TAlloc<OSCL_HeapString<PVMFEVRCParserNodeAllocator>, PVMFEVRCParserNodeAllocator> str;
        ...
        void Copy(const PVMFGenericNodeCommand<PVMFEVRCParserNodeAllocator>& aCmd)
        {
            PVMFGenericNodeCommand<PVMFEVRCParserNodeAllocator>::Copy(aCmd);
            switch (aCmd.iCmd)
            {
                case PVMF_EVRC_PARSER_NODE_GETNODEMETADATAKEYS:
                    if (aCmd.iParam4)
                    {
                        ...
                        OSCL_HeapString<PVMFEVRCParserNodeAllocator>* aStr =
                            (OSCL_HeapString<PVMFEVRCParserNodeAllocator>*)aCmd.iParam4;
                        Oscl_TAlloc<OSCL_HeapString<PVMFEVRCParserNodeAllocator>, PVMFEVRCParserNodeAllocator> str;
...
typedef PVMFNodeCommandQueue<PVMFEVRCFFNodeCommand, OsclMemAllocator> PVMFEVRCFFNodeCmdQ;
...
typedef OsclMemAllocator PVMFEVRCFFNodeAllocator;

/**
* The node class
*/
//typedef PVMFNodeCommandQueue<PVMFEVRCFFNodeCommand, PVMFEVRCParserNodeAllocator> PVMFEVRCParserNodeCmdQueue;
class CEVRCFileParser;
class PVMFEVRCFFParserOutPort;
class PVLogger;

class PVMFEVRCFFParserNode :  public OsclTimerObject
...
{
    public:
        PVMFEVRCFFParserNode(int32 aPriority = OsclActiveObject::EPriorityNominal);
        ~PVMFEVRCFFParserNode();
        ...
        PVMFCommandId QueryUUID(PVMFSessionId, const PvmfMimeString& aMimeType,
                                Oscl_Vector<PVUuid, PVMFEVRCFFNodeAllocator>& aUuids,
    ...
    private:
        ...
        void Run();
        //Command processing
        PVMFCommandId QueueCommandL(PVMFEVRCFFNodeCommand& aCmd);

        void MoveCmdToCurrentQueue(PVMFEVRCFFNodeCommand& aCmd);
        void MoveCmdToCancelQueue(PVMFEVRCFFNodeCommand& aCmd);
        bool ProcessCommand();
        void CommandComplete(PVMFEVRCFFNodeCmdQ& aCmdQueue,
        ...
        void CommandComplete(PVMFEVRCFFNodeCmdQ&, PVMFEVRCFFNodeCommand&, PVMFStatus, PVInterface*, OsclAny* aData);
        bool FlushPending();
        //Command handlers.
        void DoQueryUuid(PVMFEVRCFFNodeCommand&);
        void DoQueryInterface(PVMFEVRCFFNodeCommand&);

        PVMFStatus DoInit(PVMFEVRCFFNodeCommand&);
        void CompleteInit();

        void DoPrepare(PVMFEVRCFFNodeCommand&);
        void DoStart(PVMFEVRCFFNodeCommand&);
        void DoStop(PVMFEVRCFFNodeCommand&);
        void DoPause(PVMFEVRCFFNodeCommand&);
        void DoFlush(PVMFEVRCFFNodeCommand&);

        void DoCancelAllCommands(PVMFEVRCFFNodeCommand&);
        void DoCancelCommand(PVMFEVRCFFNodeCommand&);

        void DoRequestPort(PVMFEVRCFFNodeCommand&, PVMFPortInterface*&);
        void DoReleasePort(PVMFEVRCFFNodeCommand&);

        void DoReset(PVMFEVRCFFNodeCommand&);
        void  CompleteReset();

        // For metadata extention interface
        PVMFStatus DoGetMetadataKeys(PVMFEVRCFFNodeCommand& aCmd);
        PVMFStatus DoGetMetadataValues(PVMFEVRCFFNodeCommand& aCmd);
        ...
        PVMFStatus CompleteGetMetadataKeys(PVMFEVRCFFNodeCommand& aCmd);

        void PushToAvailableMetadataKeysList(const char* aKeystr, char* aOptionalParam = NULL);
        Oscl_Vector<OSCL_HeapString<PVMFEVRCParserNodeAllocator>, PVMFEVRCParserNodeAllocator> iAvailableMetadataKeys;
        Oscl_Vector<OSCL_HeapString<PVMFEVRCParserNodeAllocator>, PVMFEVRCParserNodeAllocator> iCPMMetadataKeys;
        uint32 iEVRCParserNodeMetadataValueCount;

        // For data source position extension interface
        PVMFStatus DoSetDataSourcePosition(PVMFEVRCFFNodeCommand& aCmd);
        PVMFStatus DoQueryDataSourcePosition(PVMFEVRCFFNodeCommand& aCmd);
        PVMFStatus DoSetDataSourceRate(PVMFEVRCFFNodeCommand& aCmd);

        /* For data source direction extension interface */
        void DoSetDataSourceDirection(PVMFEVRCFFNodeCommand& aCmd);
        ...
        bool RetrieveTrackData(PVEVRCFFNodeTrackPortInfo& aTrackPortInfo);
        PVMFStatus RetrieveTrackData(PVEVRCFFNodeTrackPortInfo& aTrackPortInfo, PVMFSharedMediaDataPtr& aMediaDataOut);
        bool SendEndOfTrackCommand(PVEVRCFFNodeTrackPortInfo& aTrackPortInfo);
        PVMFStatus SendBeginOfMediaStreamCommand(PVEVRCFFNodeTrackPortInfo* aTrackPortInfo);
        bool CheckAvailabilityForSendingNewTrackData(PVEVRCFFNodeTrackPortInfo& aTrackPortInfo);
        void ResetAllTracks();
    ...
    private: // private member variables
        ...
        PVMFEVRCFFNodeCmdQ iInputCommands;
        PVMFEVRCFFNodeCmdQ iCurrentCommand;
        PVMFEVRCFFNodeCmdQ iCancelCommand;

        PVMFEVRCFFParserOutPort* iOutPort;
        friend class PVMFEVRCFFParserOutPort;
        void ProcessOutgoingMsg();

        /* Port processing */
        PVMFPortVector<PVMFEVRCFFParserOutPort, PVMFEVRCParserNodeAllocator> iPortVector;
        bool ProcessPortActivity(PVEVRCFFNodeTrackPortInfo*);
        PVMFStatus RetrieveMediaSample(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr,
                                       PVMFSharedMediaDataPtr& aSharedPtr);
        PVMFStatus QueueMediaSample(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr);
        PVMFStatus ProcessOutgoingMsg(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr);
        bool GetTrackInfo(PVMFPortInterface* aPort,
                          PVEVRCFFNodeTrackPortInfo*& aTrackInfoPtr);
        bool GetTrackInfo(int32 aTrackID,
                          PVEVRCFFNodeTrackPortInfo*& aTrackInfoPtr);
        PVMFStatus GenerateAndSendEOSCommand(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr);
        ...
        PVMFStatus CheckForEVRCHeaderAvailability();
        void PauseAllMediaRetrieval();
        uint64 iEVRCHeaderSize;
        bool iAutoPaused;
        ...
        CEVRCFileParser* iEVRCParser;
        TPVEvrcFileInfo iEVRCFileInfo;
        ...
        Oscl_Vector<PVEVRCFFNodeTrackPortInfo, OsclMemAllocator> iSelectedTrackList;
        ...
        PVMFStatus DoGetLicense(PVMFEVRCFFNodeCommand& aCmd,
                                bool aWideCharVersion = false);
        PVMFStatus DoCancelGetLicense(PVMFEVRCFFNodeCommand& aCmd);
        void CompleteGetLicense();
        PVMFStatus ParseEVRCFile();
        uint32 iCountToClaculateRDATimeInterval;
};

#endif // PVMF_EVRCFFPARSER_NODE_H_INCLUDED
