...
#include "evrc_parsernode_tunables.h"
...
static const char PVEVRC_ALL_METADATA_KEY[] = "all";
static const char PVEVRCMETADATA_DURATION_KEY[] = "duration";
static const char PVEVRCMETADATA_NUMTRACKS_KEY[] = "num-tracks";
static const char PVEVRCMETADATA_TRACKINFO_BITRATE_KEY[] = "track-info/bit-rate";
static const char PVEVRCMETADATA_TRACKINFO_AUDIO_FORMAT_KEY[] = "track-info/audio/format";
static const char PVEVRCMETADATA_CLIP_TYPE_KEY[] = "clip-type";
static const char PVEVRCMETADATA_RANDOM_ACCESS_DENIED_KEY[] = "random-access-denied";
static const char PVEVRCMETADATA_SEMICOLON[] = ";";
static const char PVEVRCMETADATA_TIMESCALE[] = "timescale=";
static const char PVEVRCMETADATA_INDEX0[] = "index=0";

#define EVRC_SAMPLE_DURATION 20



PVMFEVRCFFParserNode::PVMFEVRCFFParserNode(int32 aPriority) :
        OsclTimerObject(aPriority, "PVEVRCFFParserNode"),
        iOutPort(NULL),
        iLogger(NULL),
        iEVRCParser(NULL),
        iExtensionRefCount(0)
{
    ...
    iEVRCParserNodeMetadataValueCount = 0;

    iDownloadProgressInterface = NULL;
    iDownloadFileSize          = 0;
    iEVRCHeaderSize             = EVRC_HEADER_SIZE;
    ...
    OSCL_TRY(err,
             ...
             iInputCommands.Construct(PVMF_EVRCFFPARSER_NODE_COMMAND_ID_START, PVMF_EVRCFFPARSER_NODE_COMMAND_VECTOR_RESERVE);
...
PVMFEVRCFFParserNode::~PVMFEVRCFFParserNode()
{
...
PVMFStatus PVMFEVRCFFParserNode::ThreadLogon()
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::ThreadLogon() Called"));
    if (iInterfaceState == EPVMFNodeCreated)
    {
        ...
        iLogger = PVLogger::GetLoggerObject("PVMFEVRCParserNode");
        iDataPathLogger = PVLogger::GetLoggerObject("datapath.sourcenode.evrcparsernode");
        iClockLogger = PVLogger::GetLoggerObject("clock");
        iFileServer.Connect();
        SetState(EPVMFNodeIdle);
        return PVMFSuccess;
    }
    PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::ThreadLogon() - Invalid State"));
    return PVMFErrInvalidState;
}

PVMFStatus PVMFEVRCFFParserNode::GetCapability(PVMFNodeCapability& aNodeCapability)
{

    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::GetCapability() called"));
...
PVMFPortIter* PVMFEVRCFFParserNode::GetPorts(const PVMFPortFilter* aFilter)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFASFParserNode::GetPorts() called"));
    OSCL_UNUSED_ARG(aFilter);
    PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFASFParserNode::GetPorts() Not Implemented"));
    // TODO: Return the currently available ports
    return NULL;
}

PVMFCommandId PVMFEVRCFFParserNode::QueryUUID(PVMFSessionId s, const PvmfMimeString& aMimeType,
        Oscl_Vector<PVUuid, OsclMemAllocator>& aUuids,
        bool aExactUuidsOnly, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::QueryUUID called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s,
                                            PVMF_EVRC_PARSER_NODE_QUERYUUID,
                                            aMimeType,
                                            aUuids,
                                            aExactUuidsOnly,
                                            aContext);
...
PVMFCommandId PVMFEVRCFFParserNode::QueryInterface(PVMFSessionId s, const PVUuid& aUuid,
        PVInterface*& aInterfacePtr,
        const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::QueryInterface called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s,
                                            PVMF_EVRC_PARSER_NODE_QUERYINTERFACE,
                                            aUuid,
                                            aInterfacePtr,
                                            aContext);
    return QueueCommandL(cmd);
...
PVMFCommandId PVMFEVRCFFParserNode::RequestPort(PVMFSessionId s, int32 aPortTag, const PvmfMimeString* aPortConfig, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::RequestPort called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s,
                                            PVMF_EVRC_PARSER_NODE_REQUESTPORT,
                                            aPortTag,
                                            aPortConfig,
                                            aContext);
    return QueueCommandL(cmd);
...
PVMFCommandId PVMFEVRCFFParserNode::ReleasePort(PVMFSessionId s, PVMFPortInterface& aPort, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::ReleasePort called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_RELEASEPORT, aPort, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Init(PVMFSessionId s, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::Init called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_INIT, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Prepare(PVMFSessionId s, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::Prepare called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_PREPARE, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Start(PVMFSessionId s, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::Start called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_START, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Stop(PVMFSessionId s, const OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:Stop"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_STOP, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Pause(PVMFSessionId s, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::Stop called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_PAUSE, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Flush(PVMFSessionId s, const OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:Flush"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_FLUSH, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::Reset(PVMFSessionId s, const OsclAny* aContext)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::Flush called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_RESET, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::CancelAllCommands(PVMFSessionId s, const OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:CancelAllCommands"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_CANCELALLCOMMANDS, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::CancelCommand(PVMFSessionId s, PVMFCommandId aCmdId, const OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:CancelCommand"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(s, PVMF_EVRC_PARSER_NODE_CANCELCOMMAND, aCmdId, aContext);
    return QueueCommandL(cmd);
}

void PVMFEVRCFFParserNode::Construct()
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::Construct()"));
    iFileServer.Connect();
    iAvailableMetadataKeys.reserve(4);
    iAvailableMetadataKeys.clear();
}

void PVMFEVRCFFParserNode::Run()
{
    ...
    {
        PVEVRCFFNodeTrackPortInfo* trackPortInfoPtr = NULL;

        if (!GetTrackInfo(iOutPort, trackPortInfoPtr))
        {
            PVMF_EVRCPARSERNODE_LOGERROR((0, "PVEVRCParserNode::Run: Error - GetTrackInfo failed"));
            return;
        }
...
PVMFStatus  PVMFEVRCFFParserNode::ProcessOutgoingMsg(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr)
{
    /*
    ...
    */
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::ProcessOutgoingMsg() Called aPort=0x%x", aTrackInfoPtr->iPort));
    PVMFStatus status = aTrackInfoPtr->iPort->Send();
    if (status == PVMFErrBusy)
    {
        /* Connected port is busy */
        aTrackInfoPtr->oProcessOutgoingMessages = false;
        PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCParserNode::ProcessOutgoingMsg() Connected port is in busy state"));
    }
    else if (status != PVMFSuccess)
    {
        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::ProcessOutgoingMsg() - aTrackInfoPtr->iPort->Send() Failed"));
    }
    return status;
}

PVMFStatus PVMFEVRCFFParserNode::DoGetMetadataKeys(PVMFEVRCFFNodeCommand& aCmd)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::DoGetNodeMetadataKeys() In"));
...
PVMFStatus
PVMFEVRCFFParserNode::CompleteGetMetadataKeys(PVMFEVRCFFNodeCommand& aCmd)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::CompleteGetMetadataKeys Called"));
    PVMFMetadataList* keylistptr = NULL;
    uint32 starting_index;
    int32 max_entries;
    char* query_key;

    aCmd.PVMFEVRCFFNodeCommand::Parse(keylistptr, starting_index, max_entries, query_key);
    if (keylistptr == NULL)
    {
...
PVMFStatus PVMFEVRCFFParserNode::DoGetMetadataValues(PVMFEVRCFFNodeCommand& aCmd)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::DoGetMetadataValues() In"));
    ...
    aCmd.PVMFEVRCFFNodeCommand::Parse(keylistptr_in, valuelistptr, starting_index, max_entries);
	...
	if (keylistptr_in->size() == 1)
	{
		if (oscl_strncmp((*keylistptr)[0].get_cstr(),
						 PVEVRC_ALL_METADATA_KEY,
						 oscl_strlen(PVEVRC_ALL_METADATA_KEY)) == 0)
		{
    ...
    for (uint32 lcv = 0; lcv < numkeys; lcv++)
    {
        ...
        if (oscl_strcmp((*keylistptr)[lcv].get_cstr(), PVEVRCMETADATA_DURATION_KEY) == 0 &&
                iEVRCFileInfo.iDuration > 0)
        {
            // Movie Duration
            ...
            if (numvalentries > starting_index)
            {
                ...
                oscl_snprintf(timescalestr, 20, ";%s%d", PVEVRCMETADATA_TIMESCALE, iEVRCFileInfo.iTimescale);
                timescalestr[19] = '\0';
                uint32 duration = Oscl_Int64_Utils::get_uint64_lower32(iEVRCFileInfo.iDuration);
                int32 retval =
                    PVMFCreateKVPUtils::CreateKVPForUInt32Value(KeyVal,
                            PVEVRCMETADATA_DURATION_KEY,
                            duration,
                            timescalestr);
        ...
        else if (oscl_strcmp((*keylistptr)[lcv].get_cstr(), PVEVRCMETADATA_NUMTRACKS_KEY) == 0)
        {
            // Number of tracks
            // Increment the counter for the number of values found so far
            ++numvalentries;
            ...
            {
                uint32 numtracks = 1;
                PVMFStatus retval = PVMFCreateKVPUtils::CreateKVPForUInt32Value(KeyVal, PVEVRCMETADATA_NUMTRACKS_KEY, numtracks);
                if (retval != PVMFSuccess && retval != PVMFErrArgument)
                {
                    break;
                }
        ...
        else if ((oscl_strcmp((*keylistptr)[lcv].get_cstr(), PVEVRCMETADATA_TRACKINFO_BITRATE_KEY) == 0) &&
                 iEVRCFileInfo.iBitrate > 0)
        {
            // Bitrate
            // Increment the counter for the number of values found so far
            ++numvalentries;
            ...
            {
                char indexparam[16];
                oscl_snprintf(indexparam, 16, ";%s", PVEVRCMETADATA_INDEX0);
                indexparam[15] = '\0';
                uint32 bitrate = iEVRCFileInfo.iBitrate;
                retval = PVMFCreateKVPUtils::CreateKVPForUInt32Value(KeyVal, PVEVRCMETADATA_TRACKINFO_BITRATE_KEY, bitrate, indexparam);
            }
            if (retval != PVMFSuccess && retval != PVMFErrArgument)
            {
                break;
        ...
        else if (oscl_strcmp((*keylistptr)[lcv].get_cstr(), PVEVRCMETADATA_RANDOM_ACCESS_DENIED_KEY) == 0)
        {
            /*
             * Random Access
             * Increment the counter for the number of values found so far
            ...
            if (numvalentries > (uint32)starting_index)
            {
                ...
                PVMFStatus retval =
                    PVMFCreateKVPUtils::CreateKVPForBoolValue(KeyVal,
                            PVEVRCMETADATA_RANDOM_ACCESS_DENIED_KEY,
        ...
        else if (oscl_strncmp((*keylistptr)[lcv].get_cstr(), PVEVRCMETADATA_CLIP_TYPE_KEY, oscl_strlen(PVEVRCMETADATA_CLIP_TYPE_KEY)) == 0)
        {
            /*
             * Clip Type
             * Increment the counter for the number of values found so far
        ...
        else if (oscl_strcmp((*keylistptr)[lcv].get_cstr(), PVEVRCMETADATA_TRACKINFO_AUDIO_FORMAT_KEY) == 0)
        {
            // Format
            // Increment the counter for the number of values found so far
            ++numvalentries;
            ...
            if (numvalentries > starting_index)
            {
                ...
                retval = PVMFCreateKVPUtils::CreateKVPForCharStringValue(KeyVal,
                               PVEVRCMETADATA_TRACKINFO_AUDIO_FORMAT_KEY, _STRLIT_CHAR(PVMF_MIME_EVRC), indexparam);
    ...
    iEVRCParserNodeMetadataValueCount = (*valuelistptr).size();
...
PVMFStatus PVMFEVRCFFParserNode::DoSetDataSourcePosition(PVMFEVRCFFNodeCommand& aCmd)
{
    //file must be parsed
    if (!iEVRCParser)
    {
    ...
    aCmd.PVMFEVRCFFNodeCommand::Parse(targetNPT, actualNPT, actualMediaDataTS, seektosyncpoint, streamID);

    Oscl_Vector<PVEVRCFFNodeTrackPortInfo, PVMFEVRCParserNodeAllocator>::iterator it;
    ...
    *actualMediaDataTS = 0;
    ...
    uint32 timestamp;
    int32 result = iEVRCParser->PeekNextTimestamp(&timestamp);
    if (result != evrcbitstreamObject::EVERYTHING_OK)
    {
    ...
    *actualMediaDataTS = millisecTS;
    ...
    uint32 durationms = 0;
    uint32 duration = durationms = Oscl_Int64_Utils::get_uint64_lower32(iEVRCFileInfo.iDuration);
    uint32 timescale = iEVRCFileInfo.iTimescale;
    ...
    if (targetNPT >= durationms)
    {
        ...
        result = iEVRCParser->ResetPlayback(0);
        if (result != evrcbitstreamObject::EVERYTHING_OK)
        {
            return PVMFErrResource;
        }
    ...
    // Reposition
    // If new position is past the end of clip, EVRC FF should set the position to the last frame
    result = iEVRCParser->ResetPlayback(targetNPT);
    if (result != evrcbitstreamObject::EVERYTHING_OK)
    {
        if (evrcbitstreamObject::END_OF_FILE == result)
        {
            for (uint32 i = 0; i < iSelectedTrackList.size(); ++i)
            {
                iSelectedTrackList[i].iSeqNum = 0;
    ...
    uint32 newtimestamp;
    result = iEVRCParser->PeekNextTimestamp(&newtimestamp);
    if (result != evrcbitstreamObject::EVERYTHING_OK)
    {
        return PVMFErrResource;
    }
...
PVMFStatus PVMFEVRCFFParserNode::DoQueryDataSourcePosition(PVMFEVRCFFNodeCommand& aCmd)
{
    //file must be parsed
    if (!iEVRCParser)
    {
    ...
    aCmd.PVMFEVRCFFNodeCommand::Parse(targetNPT, actualNPT, seektosyncpoint);
    if (actualNPT == NULL)
    {
        return PVMFErrArgument;
    }
    ...
    // Query
    // If new position is past the end of clip, EVRC FF should set the position to the last frame
    *actualNPT = iEVRCParser->SeekPointFromTimestamp(targetNPT);

    return PVMFSuccess;
}

PVMFStatus PVMFEVRCFFParserNode::DoSetDataSourceRate(PVMFEVRCFFNodeCommand& aCmd)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::DoSetDataSourceRate() In"));
    OSCL_UNUSED_ARG(aCmd);
    return PVMFSuccess;
}

bool PVMFEVRCFFParserNode::SendEndOfTrackCommand(PVEVRCFFNodeTrackPortInfo& aTrackPortInfo)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::SendEndOfTrackCommand() "));
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::SendEndOfTrackCommand() Failed-- Busy "));
        return false;
    }
...
void PVMFEVRCFFParserNode::HandlePortActivity(const PVMFPortActivity &aActivity)
{
    ...
    switch (aActivity.iType)
    {
        case PVMF_PORT_ACTIVITY_OUTGOING_MSG:
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFEVRCFFParserNode::PortActivity: Outgoing Msg"));
            RunIfNotReady();
            break;
        ...
        case PVMF_PORT_ACTIVITY_CONNECTED_PORT_READY:
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFEVRCFFParserNode::PortActivity: Connected port ready"));
        ...
        case PVMF_PORT_ACTIVITY_OUTGOING_QUEUE_READY:
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFEVRCFFParserNode::PortActivity: Outgoing Queue ready"));
...
bool PVMFEVRCFFParserNode::ProcessCommand()
{
    ...
    if (!iCurrentCommand.empty()  && !iInputCommands.front().hipri() && iInputCommands.front().iCmd != PVMF_EVRC_PARSER_NODE_CMD_CANCEL_GET_LICENSE)
    {
        return false;
    }
    ...
    PVMFEVRCFFNodeCommand& aCmd = iInputCommands.front();
    ...
    if (aCmd.hipri())
    {
        switch (aCmd.iCmd)
        {
            case PVMF_EVRC_PARSER_NODE_CANCELALLCOMMANDS:
                DoCancelAllCommands(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_CANCELCOMMAND:
    ...
    else
    {
        ...
        switch (aCmd.iCmd)
        {
            case PVMF_EVRC_PARSER_NODE_QUERYUUID:
                DoQueryUuid(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_QUERYINTERFACE:
                DoQueryInterface(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_REQUESTPORT:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_RELEASEPORT:
                DoReleasePort(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_INIT:
            ...
            case PVMF_EVRC_PARSER_NODE_PREPARE:
                DoPrepare(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_START:
                DoStart(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_STOP:
                DoStop(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_FLUSH:
                DoFlush(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_PAUSE:
                DoPause(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_RESET:
                DoReset(aCmd);
                break;

            case PVMF_EVRC_PARSER_NODE_GETNODEMETADATAKEYS:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_GETNODEMETADATAVALUES:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_POSITION:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_QUERY_DATASOURCE_POSITION:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_RATE:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_GET_LICENSE_W:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_GET_LICENSE:
            {
            ...
            case PVMF_EVRC_PARSER_NODE_CMD_CANCEL_GET_LICENSE:
...
void PVMFEVRCFFParserNode::SetState(TPVMFNodeInterfaceState s)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:SetState"));
    PVMFNodeInterface::SetState(s);
}

void PVMFEVRCFFParserNode::ReportErrorEvent(PVMFEventType aEventType, OsclAny* aEventData, PVUuid* aEventUUID, int32* aEventCode)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:ReportErrorEvent Type %d Data %d"
                    , aEventType, aEventData));

    if (aEventUUID && aEventCode)
    {
        PVMFBasicErrorInfoMessage* eventmsg;
        PVMF_EVRC_PARSER_NODE_NEW(NULL,
...
void PVMFEVRCFFParserNode::ReportInfoEvent(PVMFEventType aEventType, OsclAny* aEventData, PVUuid* aEventUUID, int32* aEventCode)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:ReportInfoEvent Type %d Data %d"
                    , aEventType, aEventData));
...
void PVMFEVRCFFParserNode::DoQueryUuid(PVMFEVRCFFNodeCommand& aCmd)
{
    OSCL_String* mimetype;
    Oscl_Vector<PVUuid, OsclMemAllocator> *uuidvec;
    bool exactmatch;
    aCmd.PVMFEVRCFFNodeCommandBase::Parse(mimetype, uuidvec, exactmatch);
...
void PVMFEVRCFFParserNode::DoQueryInterface(PVMFEVRCFFNodeCommand&  aCmd)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFEVRCFFParserNode::DoQueryInterface"));

    PVUuid* uuid;
    PVInterface** ptr;
    aCmd.PVMFEVRCFFNodeCommandBase::Parse(uuid, ptr);
...
PVMFStatus PVMFEVRCFFParserNode::DoInit(PVMFEVRCFFNodeCommand& aCmd)
{
    OSCL_UNUSED_ARG(aCmd);
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::DoInitNode() In"));
    ...
    {
        if (CheckForEVRCHeaderAvailability() == PVMFSuccess)
        {
            ParseEVRCFile();
...
PVMFStatus PVMFEVRCFFParserNode::ParseEVRCFile()
{
    iEVRCParser = OSCL_NEW(CEVRCFileParser, ());
    if (!iEVRCParser)
    {
    ...
    if (iEVRCParser->InitEVRCFile(iSourceURL, calcDuration, &iFileServer, dsFactory, iFileHandle, iCountToClaculateRDATimeInterval))
    {
        iAvailableMetadataKeys.clear();
        if (iEVRCParser->RetrieveFileInfo(iEVRCFileInfo))
        {
            ...
            else
            {
                ...
                PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::ParseEVRCFile() - InitMetaData Failed"));
...
void PVMFEVRCFFParserNode::DoPrepare(PVMFEVRCFFNodeCommand& aCmd)
{
...
void PVMFEVRCFFParserNode::DoStart(PVMFEVRCFFNodeCommand& aCmd)
{
...
void PVMFEVRCFFParserNode::DoStop(PVMFEVRCFFNodeCommand& aCmd)
{
    iStreamID = 0;
    ...
    ResetAllTracks();
            ...
            // Reset the EVRC FF to beginning
            if (iEVRCParser)
            {
                iEVRCParser->ResetPlayback(0);
            }
...
void PVMFEVRCFFParserNode::DoPause(PVMFEVRCFFNodeCommand& aCmd)
{
    if (iInterfaceState != EPVMFNodeStarted)
    {
        CommandComplete(iInputCommands, aCmd, PVMFErrInvalidState);
...
void PVMFEVRCFFParserNode::DoFlush(PVMFEVRCFFNodeCommand& aCmd)
{
...
bool PVMFEVRCFFParserNode::FlushPending()
{
    return (iCurrentCommand.size() > 0
            && iCurrentCommand.front().iCmd == PVMF_EVRC_PARSER_NODE_FLUSH);
}

void PVMFEVRCFFParserNode::DoReset(PVMFEVRCFFNodeCommand& aCmd)
{

    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::DoReset() Called"));
...
void
PVMFEVRCFFParserNode::MoveCmdToCurrentQueue(PVMFEVRCFFNodeCommand& aCmd)
{
    int32 err;
...
void
PVMFEVRCFFParserNode::MoveCmdToCancelQueue(PVMFEVRCFFNodeCommand& aCmd)
{
    /*
...
void PVMFEVRCFFParserNode::DoCancelAllCommands(PVMFEVRCFFNodeCommand& aCmd)
{
    while (!iCurrentCommand.empty())
    {
        MoveCmdToCancelQueue(aCmd);
...
void PVMFEVRCFFParserNode::DoCancelCommand(PVMFEVRCFFNodeCommand& aCmd)
{
    PVMFCommandId id;
    aCmd.PVMFEVRCFFNodeCommandBase::Parse(id);
    {
        PVMFEVRCFFNodeCommand* cmd = iCurrentCommand.FindById(id);
        if (cmd)
        {
    ...
    {
        PVMFEVRCFFNodeCommand* cmd = iInputCommands.FindById(id, 1);
        if (cmd)
        {
            //cancel the queued command
...
void PVMFEVRCFFParserNode::DoRequestPort(PVMFEVRCFFNodeCommand& aCmd, PVMFPortInterface*&aPort)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::DoRequestPort() In"));
    aPort = NULL;

    if ((iInterfaceState != EPVMFNodePrepared) || (!iEVRCParser))
    {
        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFASFParserNode::DoRequestPort() - Invalid State"));
        CommandComplete(iInputCommands, aCmd, PVMFErrInvalidState);
        return;
    }
    ...
    aCmd.PVMFEVRCFFNodeCommandBase::Parse(tag, mime_string);

    if (tag != PVMF_EVRCFFPARSER_NODE_PORT_TYPE_SOURCE)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_ERR,
                        (0, "PVMFEVRCFFParserNode::DoRequestPort: Error - Invalid port tag"));
        CommandComplete(iInputCommands, aCmd, PVMFFailure);
        return;
    }
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                        (0, "PVMFEVRCFFParserNode::DoRequestPort: Error - port already exists"));
        CommandComplete(iInputCommands, aCmd, PVMFFailure);
        return;
    }

    if ((int32)aCmd.iParam1 == PVMF_EVRCFFPARSER_NODE_PORT_TYPE_SOURCE)
    {

        iOutPort = OSCL_NEW(PVMFEVRCFFParserOutPort, (PVMF_EVRCFFPARSER_NODE_PORT_TYPE_SOURCE, this));
        if (!iOutPort)
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFEVRCFFParserNode::DoRequestPort: Error - no memory"));
            CommandComplete(iInputCommands, aCmd, PVMFErrNoMemory);
            return;
        }
        if (mime_string)
        {
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                                (0, "PVMFEVRCFFParserNode::DoRequestPort: Error - format not supported"));
        ...
        int32 leavecode = 0;
        OSCL_TRY(leavecode,
                 clockconv = OSCL_NEW(MediaClockConverter, (iEVRCFileInfo.iTimescale));
                 trackdatamempool = OSCL_NEW(OsclMemPoolFixedChunkAllocator, (PVEVRCFF_MEDIADATA_POOLNUM));
                 mediadataimplalloc = OSCL_NEW(PVMFSimpleMediaBufferCombinedAlloc, (trackdatamempool));
                 mediadatamempool = OSCL_NEW(PVMFMemPoolFixedChunkAllocator, ("EvrcFFPar", PVEVRCFF_MEDIADATA_POOLNUM, PVEVRCFF_MEDIADATA_CHUNKSIZE));
                );

        if (leavecode || !clockconv || !trackdatamempool || !mediadataimplalloc || !mediadatamempool)
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFEVRCFFParserNode::DoRequestPort: Error - unable to create clockconv, trackdatamempool, mediadataimplalloc, and mediadatamempool"));
            if (iOutPort)
            {
                OSCL_DELETE(iOutPort);
                iOutPort = NULL;
        ...
        PVEVRCFFNodeTrackPortInfo trackportinfo;

        trackportinfo.iTrackId = 0;  // Only support 1 channel so far
        trackportinfo.iTag = PVMF_EVRCFFPARSER_NODE_PORT_TYPE_SOURCE;
        ...
        trackportinfo.iResizableDataMemoryPoolSize = PVMF_EVRC_PARSER_NODE_MAX_AUDIO_DATA_MEM_POOL_SIZE;
        PVMF_EVRC_PARSER_NODE_NEW(NULL,
                                 OsclMemPoolResizableAllocator,
                                 (trackportinfo.iResizableDataMemoryPoolSize,
                                  PVMF_EVRC_PARSER_NODE_DATA_MEM_POOL_GROWTH_LIMIT),
                                 trackDataResizableMemPool);

        PVUuid eventuuid = PVMFEVRCParserNodeEventTypesUUID;
        int32	errcode = PVMFEVRCFFParserErrTrackMediaMsgAllocatorCreationFailed;
        ...
        {
            PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::DoRequestPort() - trackDataResizableMemPool Alloc Failed"));
        ...
        uint8* typeSpecificInfoBuff = iEVRCParser->getCodecSpecificInfo();
        uint32 typeSpecificDataLength = EVRC_MAX_NUM_PACKED_INPUT_BYTES;
        if ((int32)typeSpecificDataLength > 0)
        {
            ...
            {
                PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::PopulateTrackInfoVec - Unable to Allocate Memory"));
            }
    ...
    else
    {
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                        (0, "PVMFEVRCFFParserNode::DoRequestPort: Error - type not supported"));
        CommandComplete(iInputCommands, aCmd, PVMFFailure);
        return;
    }
...
void PVMFEVRCFFParserNode::DoReleasePort(PVMFEVRCFFNodeCommand& aCmd)
{

    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFEVRCFFParserNode::DoReleasePort"));
    ...
    for (uint32 i = 0;i < iSelectedTrackList.size();i++)
    {
        if (iSelectedTrackList[i].iPort == aCmd.iParam1)
        {
            ...
            OSCL_DELETE(((PVMFEVRCFFParserOutPort*)iSelectedTrackList[i].iPort));
            ...
            {
                PVMF_EVRC_PARSER_NODE_DELETE(NULL,
                                            PVMFResizableSimpleMediaMsgAlloc,
...
void PVMFEVRCFFParserNode::ResetAllTracks()
{
    for (uint32 i = 0; i < iSelectedTrackList.size(); ++i)
    {
        iSelectedTrackList[i].iMediaData.Unbind();
...
void PVMFEVRCFFParserNode::CleanupFileSource()
{
    iAvailableMetadataKeys.clear();
...
void PVMFEVRCFFParserNode::CommandComplete(PVMFEVRCFFNodeCmdQ& aCmdQ, PVMFEVRCFFNodeCommand& aCmd, PVMFStatus aStatus, PVInterface*aExtMsg, OsclAny* aEventData)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_INFO, (0, "PVMFEVRCFFParserNode:CommandComplete Id %d Cmd %d Status %d Context %d Data %d"
...
void PVMFEVRCFFParserNode::CommandComplete(PVMFEVRCFFNodeCmdQ& aCmdQ,
        PVMFEVRCFFNodeCommand& aCmd,
        PVMFStatus aStatus,
        OsclAny* aEventData,
        PVUuid* aEventUUID,
        int32* aEventCode,
        PVInterface* aExtMsg)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::CommandComplete() In Id %d Cmd %d Status %d Context %d Data %d",
                                      aCmd.iId, aCmd.iCmd, aStatus, aCmd.iContext, aEventData));
...
PVMFCommandId PVMFEVRCFFParserNode::QueueCommandL(PVMFEVRCFFNodeCommand& aCmd)
{
    if (IsAdded())
    {
        PVMFCommandId id;
...
void PVMFEVRCFFParserNode::addRef()
{
    ++iExtensionRefCount;
}

void PVMFEVRCFFParserNode::removeRef()
{
    --iExtensionRefCount;
}

PVMFStatus PVMFEVRCFFParserNode::QueryInterfaceSync(PVMFSessionId aSession,
        const PVUuid& aUuid,
        PVInterface*& aInterfacePtr)
{
...
bool PVMFEVRCFFParserNode::queryInterface(const PVUuid& uuid, PVInterface*& iface)
{
...
PVMFStatus PVMFEVRCFFParserNode::SetSourceInitializationData(OSCL_wString& aSourceURL, PVMFFormatType& aSourceFormat, OsclAny* aSourceData)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::SetSourceInitializationData() called"));
    if (aSourceFormat == PVMF_MIME_EVRCFF)
    {
    ...
    PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::SetSourceInitializationData - Unsupported Format"));
    return PVMFFailure;
}

PVMFStatus PVMFEVRCFFParserNode::SetClientPlayBackClock(PVMFMediaClock* aClientClock)
{
    OSCL_UNUSED_ARG(aClientClock);
    return PVMFSuccess;
}

PVMFStatus PVMFEVRCFFParserNode::SetEstimatedServerClock(PVMFMediaClock* aClientClock)
{
    OSCL_UNUSED_ARG(aClientClock);
    return PVMFSuccess;
}

PVMFStatus PVMFEVRCFFParserNode::GetMediaPresentationInfo(PVMFMediaPresentationInfo& aInfo)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::GetMediaPresentationInfo() called"));
    ...
    aInfo.setDurationValue(iEVRCFileInfo.iDuration);
    // Current version of AAC parser is limited to 1 channel
    PVMFTrackInfo tmpTrackInfo;
    tmpTrackInfo.setPortTag(PVMF_EVRCFFPARSER_NODE_PORT_TYPE_SOURCE);
    tmpTrackInfo.setTrackID(0);
    TPVEvrcFileInfo evrcinfo;
    if (!iEVRCParser->RetrieveFileInfo(evrcinfo)) return PVMFErrNotSupported;

    tmpTrackInfo.setTrackBitRate(evrcinfo.iBitrate);
    tmpTrackInfo.setTrackDurationTimeScale((uint64)evrcinfo.iTimescale);
    tmpTrackInfo.setTrackDurationValue(evrcinfo.iDuration);
    OSCL_FastString mime_type = _STRLIT_CHAR(PVMF_MIME_EVRC);
...
PVMFStatus PVMFEVRCFFParserNode::SelectTracks(PVMFMediaPresentationInfo& aInfo)
{
    OSCL_UNUSED_ARG(aInfo);
    return PVMFSuccess;
}

uint32 PVMFEVRCFFParserNode::GetNumMetadataKeys(char* aQueryKeyString)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::GetNumMetadataKeys() called"));
...
uint32 PVMFEVRCFFParserNode::GetNumMetadataValues(PVMFMetadataList& aKeyList)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::GetNumMetadataValues() called"));

    uint32 numkeys = aKeyList.size();
    if (!iEVRCParser || numkeys == 0)
    {
        return 0;
    }
    ...
    for (uint32 lcv = 0; lcv < numkeys; lcv++)
    {
        if (oscl_strcmp(aKeyList[lcv].get_cstr(), PVEVRCMETADATA_DURATION_KEY) == 0 &&
                iEVRCFileInfo.iDuration > 0)
        {
            // Movie Duration
            ++numvalentries;
        }
        else if (oscl_strcmp(aKeyList[lcv].get_cstr(), PVEVRCMETADATA_NUMTRACKS_KEY) == 0)
        {
            // Number of tracks
            ++numvalentries;
        }
        else if ((oscl_strcmp(aKeyList[lcv].get_cstr(), PVEVRCMETADATA_TRACKINFO_BITRATE_KEY) == 0) &&
                 iEVRCFileInfo.iBitrate > 0)
        {
            // Bitrate
            ++numvalentries;
        }
        else if (oscl_strcmp(aKeyList[lcv].get_cstr(), PVEVRCMETADATA_TRACKINFO_AUDIO_FORMAT_KEY))
        {
            // Format
            ++numvalentries;
        }
        else if (oscl_strcmp(aKeyList[lcv].get_cstr(), PVEVRCMETADATA_RANDOM_ACCESS_DENIED_KEY) == 0)
        {
            /*
             * Random Access
             * Increment the counter for the number of values found so far
        ...
        else if (oscl_strcmp(aKeyList[lcv].get_cstr(), PVEVRCMETADATA_CLIP_TYPE_KEY) == 0)
        {
            /*
             * clip-type
             * Increment the counter for the number of values found so far
...
PVMFCommandId PVMFEVRCFFParserNode::GetNodeMetadataKeys(PVMFSessionId aSessionId, PVMFMetadataList& aKeyList
        , uint32 aStartingKeyIndex, int32 aMaxKeyEntries, char* aQueryKeyString, const OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::GetNodeMetadataKeys() called"));

    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_GETNODEMETADATAKEYS, aKeyList, aStartingKeyIndex, aMaxKeyEntries, aQueryKeyString, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::GetNodeMetadataValues(PVMFSessionId aSessionId, PVMFMetadataList& aKeyList, Oscl_Vector<PvmiKvp, OsclMemAllocator>& aValueList
        , uint32 aStartingValueIndex, int32 aMaxValueEntries, const OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::GetNodeMetadataValue() called"));

    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_GETNODEMETADATAVALUES, aKeyList, aValueList, aStartingValueIndex, aMaxValueEntries, aContext);
    return QueueCommandL(cmd);
}

PVMFStatus PVMFEVRCFFParserNode::ReleaseNodeMetadataKeys(PVMFMetadataList& , uint32 , uint32)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::ReleaseNodeMetadataKeys() called"));
    return PVMFSuccess;
}

PVMFStatus PVMFEVRCFFParserNode::ReleaseNodeMetadataValues(Oscl_Vector<PvmiKvp, OsclMemAllocator>& aValueList, uint32 start, uint32 end)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::ReleaseNodeMetadataValues() called"));

    if (start > end || aValueList.size() == 0)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMFEVRCFFParserNode::ReleaseNodeMetadataValues() Invalid start/end index"));
        return PVMFErrArgument;
    }

    end = OSCL_MIN(aValueList.size(), iEVRCParserNodeMetadataValueCount);
...
PVMFCommandId PVMFEVRCFFParserNode::SetDataSourcePosition(PVMFSessionId aSessionId
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFEVRCFFParserNode::SetDataSourcePosition: aTargetNPT=%d, aSeekToSyncPoint=%d, aContext=0x%x",
                     aTargetNPT, aSeekToSyncPoint, aContext));

    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_POSITION, aTargetNPT, aActualNPT,
                                        aActualMediaDataTS, aSeekToSyncPoint, aStreamID, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::QueryDataSourcePosition(PVMFSessionId aSessionId
        , PVMFTimestamp aTargetNPT
        , PVMFTimestamp& aActualNPT
        , bool aSeekToSyncPoint
        , OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFEVRCFFParserNode::QueryDataSourcePosition: aTargetNPT=%d, aSeekToSyncPoint=%d, aContext=0x%x",
                     aTargetNPT, aSeekToSyncPoint, aContext));

    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_QUERY_DATASOURCE_POSITION, aTargetNPT, aActualNPT,
        ...
        , bool aSeekToSyncPoint)
{
    ...
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_QUERY_DATASOURCE_POSITION, aTargetNPT, aSeekPointBeforeTargetNPT,
                                        aSeekToSyncPoint, aContext);
    return QueueCommandL(cmd);
}

PVMFCommandId PVMFEVRCFFParserNode::SetDataSourceRate(PVMFSessionId aSessionId
        , int32 aRate
        , PVMFTimebase* aTimebase
        , OsclAny* aContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::SetDataSourceRate() called"));

    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_SET_DATASOURCE_RATE, aRate, aTimebase, aContext);
    return QueueCommandL(cmd);
}


PVMFStatus PVMFEVRCFFParserNode::CheckForEVRCHeaderAvailability()
{
    if (iDataStreamInterface != NULL)
    {
        /*
        ...
        if (currCapacity <  EVRC_MIN_DATA_SIZE_FOR_RECOGNITION)
        {
            iRequestReadCapacityNotificationID =
                iDataStreamInterface->RequestReadCapacityNotification(iDataStreamSessionID,
                        *this,
                        EVRC_MIN_DATA_SIZE_FOR_RECOGNITION);
            return PVMFPending;
        }


        uint32 headerSize32 =
            Oscl_Int64_Utils::get_uint64_lower32(iEVRCHeaderSize);
...
bool PVMFEVRCFFParserNode::GetTrackInfo(PVMFPortInterface* aPort,
                                       PVEVRCFFNodeTrackPortInfo*& aTrackInfoPtr)
{
    Oscl_Vector<PVEVRCFFNodeTrackPortInfo, PVMFEVRCParserNodeAllocator>::iterator it;
    for (it = iSelectedTrackList.begin(); it != iSelectedTrackList.end(); it++)
    {
...
bool PVMFEVRCFFParserNode::GetTrackInfo(int32 aTrackID,
                                       PVEVRCFFNodeTrackPortInfo*& aTrackInfoPtr)
{
    Oscl_Vector<PVEVRCFFNodeTrackPortInfo, PVMFEVRCParserNodeAllocator>::iterator it;
    for (it = iSelectedTrackList.begin(); it != iSelectedTrackList.end(); it++)
...
bool PVMFEVRCFFParserNode::ProcessPortActivity(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr)
{
    /*
     * called by the AO to process a port activity message
     */
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::ProcessPortActivity() Called"));
    ...
    if (aTrackInfoPtr->oQueueOutgoingMessages)
    {
        ...
        {
            PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::ProcessPortActivity() QueueMediaSample Failed - Err=%d", status));
            return false;
        }
        if (iAutoPaused == true)
        {
            aTrackInfoPtr->oQueueOutgoingMessages = false;
            PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCParserNode::QueueMediaSample() - Auto Paused"));
            return PVMFErrBusy;
        }
        if (aTrackInfoPtr->iPort->IsOutgoingQueueBusy())
        {
            aTrackInfoPtr->oQueueOutgoingMessages = false;
            PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCParserNode::QueueMediaSample() Port Outgoing Queue Busy"));
            return PVMFErrBusy;
        }
    ...
    if (aTrackInfoPtr->oProcessOutgoingMessages)
    {
        if (aTrackInfoPtr->iPort->OutgoingMsgQueueSize() > 0)
        {
            ...
            if ((status != PVMFErrBusy) &&
                    ...
                    (status != PVMFErrInvalidState))
            {
                PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFASFParserNode::ProcessPortActivity() ProcessOutgoingMsg Failed - Err=%d", status));
...
bool PVMFEVRCFFParserNode::CheckForPortRescheduling()
{
    PVEVRCFFNodeTrackPortInfo* trackInfoPtr = NULL;
    if (!GetTrackInfo(iOutPort, trackInfoPtr))
    {
        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::CheckForPortRescheduling: Error - GetPortContainer failed"));
        return false;
    }
...
PVMFStatus PVMFEVRCFFParserNode::InitMetaData()
{
    if (iEVRCFileInfo.iFileSize > 0)
    {
        // Populate the metadata key vector based on info available
        PushToAvailableMetadataKeysList(PVEVRCMETADATA_NUMTRACKS_KEY);
        if (iEVRCFileInfo.iDuration > 0)
        {
            PushToAvailableMetadataKeysList(PVEVRCMETADATA_DURATION_KEY);

        }
        if (iEVRCFileInfo.iBitrate > 0)
        {
            PushToAvailableMetadataKeysList(PVEVRCMETADATA_TRACKINFO_BITRATE_KEY);

        }
        PushToAvailableMetadataKeysList(PVEVRCMETADATA_TRACKINFO_AUDIO_FORMAT_KEY);
        PushToAvailableMetadataKeysList(PVEVRCMETADATA_RANDOM_ACCESS_DENIED_KEY);
        PushToAvailableMetadataKeysList(PVEVRCMETADATA_CLIP_TYPE_KEY);

        //set clip duration on download progress interface
        //applicable to PDL sessions
        {
            if ((iDownloadProgressInterface != NULL) && (iEVRCFileInfo.iDuration != 0))
            {
                iDownloadProgressInterface->setClipDuration(OSCL_CONST_CAST(uint32, iEVRCFileInfo.iDuration));
            }
        }
...
void PVMFEVRCFFParserNode::PushToAvailableMetadataKeysList(const char* aKeystr, char* aOptionalParam)
{
    if (aKeystr == NULL)
    {
        return;
...
void PVMFEVRCFFParserNode::InitCPM()
{
    iCPMInitCmdId = iCPM->Init();
}

void PVMFEVRCFFParserNode::OpenCPMSession()
{
    iCPMOpenSessionCmdId = iCPM->OpenSession(iCPMSessionID);
}

void PVMFEVRCFFParserNode::CPMRegisterContent()
{
...
void PVMFEVRCFFParserNode::GetCPMLicenseInterface()
{
...
bool PVMFEVRCFFParserNode::GetCPMContentAccessFactory()
{
...
bool PVMFEVRCFFParserNode::GetCPMMetaDataExtensionInterface()
{
...
void PVMFEVRCFFParserNode::RequestUsage()
{
    PopulateDRMInfo();
...
void PVMFEVRCFFParserNode::PopulateDRMInfo()
{
    if (iRequestedUsage.key)
    {
        OSCL_ARRAY_DELETE(iRequestedUsage.key);
...
void PVMFEVRCFFParserNode::SendUsageComplete()
{
    iCPMUsageCompleteCmdId = iCPM->UsageComplete(iCPMSessionID, iUsageID);
}

void PVMFEVRCFFParserNode::CloseCPMSession()
{
    iCPMCloseSessionCmdId = iCPM->CloseSession(iCPMSessionID);
}

void PVMFEVRCFFParserNode::ResetCPM()
{
    iCPMResetCmdId = iCPM->Reset();
}

void PVMFEVRCFFParserNode::GetCPMMetaDataKeys()
{
    if (iCPMMetaDataExtensionInterface != NULL)
    {
        ...
        iCPMGetMetaDataKeysCmdId =
            iCPMMetaDataExtensionInterface->GetNodeMetadataKeys(iCPMSessionID,
                    iCPMMetadataKeys,
                    0,
                    PVMF_EVRC_PARSER_NODE_MAX_CPM_METADATA_KEYS);
    }
}

void PVMFEVRCFFParserNode::CPMCommandCompleted(const PVMFCmdResp& aResponse)
{
    ...
    if (id == iCPMCancelGetLicenseCmdId)
    {
         ...
         */
        PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCFFParserNode::CPMCommandCompleted -  CPM CancelGetLicense complete"));
    ...
    else if (id == iCPMRegisterContentCmdId && status == PVMFErrNotSupported)
    {
        ...
        PVMF_EVRCPARSERNODE_LOGINFO((0, "PVMFEVRCParserNode::CPMCommandCompleted - Unknown CPM Format - Ignoring CPM"));
        if (CheckForEVRCHeaderAvailability() == PVMFSuccess)
        {
            if (ParseEVRCFile())
            {
                ...
                OSCL_ASSERT(iCurrentCommand.front().iCmd == PVMF_EVRC_PARSER_NODE_INIT);
                CompleteInit();
            }
        }
        return;
    ...
    else
    {
        ...
        else if (id == iCPMResetCmdId)
        {
            ...
            OSCL_ASSERT(!iCurrentCommand.empty());
            OSCL_ASSERT(iCurrentCommand.front().iCmd == PVMF_EVRC_PARSER_NODE_RESET);
            CompleteReset();
        }
        else if (id == iCPMGetMetaDataValuesCmdId)
        {
            ...
            OSCL_ASSERT(!iCurrentCommand.empty());
            OSCL_ASSERT(iCurrentCommand.front().iCmd == PVMF_EVRC_PARSER_NODE_GETNODEMETADATAVALUES);
    ...
    if (!iCancelCommand.empty())
    {
        if (iCancelCommand.front().iCmd != PVMF_EVRC_PARSER_NODE_CMD_CANCEL_GET_LICENSE)
        {
            CommandComplete(iCancelCommand,
                            iCancelCommand.front(),
                            PVMFSuccess);
...
void PVMFEVRCFFParserNode::PassDatastreamFactory(PVMFDataStreamFactory& aFactory,
        int32 aFactoryTag,
...
void
PVMFEVRCFFParserNode::PassDatastreamReadCapacityObserver(PVMFDataStreamReadCapacityObserver* aObserver)
{
    iDataStreamReadCapacityObserver = aObserver;
}

void PVMFEVRCFFParserNode::CompleteInit()
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::CompleteInit() Called"));
    if (iCPM)
    {
...
void PVMFEVRCFFParserNode::CompleteGetMetaDataValues()
{
    CommandComplete(iCurrentCommand,
                    iCurrentCommand.front(),
                    PVMFSuccess);
...
void PVMFEVRCFFParserNode::setFileSize(const uint32 aFileSize)
{
    iDownloadFileSize = aFileSize;
}

int32 PVMFEVRCFFParserNode::convertSizeToTime(uint32 aFileSize, uint32& aNPTInMS)
{
...
void PVMFEVRCFFParserNode::setDownloadProgressInterface(PVMFDownloadProgressInterface* aInterface)
{
    if (aInterface == NULL)
    {
        OSCL_ASSERT(false);
...
void PVMFEVRCFFParserNode::DataStreamInformationalEvent(const PVMFAsyncEvent& aEvent)
{
    OSCL_UNUSED_ARG(aEvent);
    OSCL_LEAVE(OsclErrNotSupported);
}

void PVMFEVRCFFParserNode::DataStreamErrorEvent(const PVMFAsyncEvent& aEvent)
{
    OSCL_UNUSED_ARG(aEvent);
    OSCL_LEAVE(OsclErrNotSupported);
}

void PVMFEVRCFFParserNode::DataStreamCommandCompleted(const PVMFCmdResp& aResponse)
{
    if (aResponse.GetCmdId() == iRequestReadCapacityNotificationID)
    {
        ...
        {
            PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::DataStreamCommandCompleted() Failed %d", cmdStatus));
            CommandComplete(iCurrentCommand,
                            iCurrentCommand.front(),
                            PVMFErrResource);
...
void PVMFEVRCFFParserNode::playResumeNotification(bool aDownloadComplete)
{
    iAutoPaused = false;
    ...
    if (!GetTrackInfo(iOutPort, trackInfoPtr))
    {
        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFASFParserNode::playResumeNotification: Error - GetPortContainer failed"));
        return;
    }
    ...
    PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::playResumeNotification() - Auto Resume Triggered - FileSize = %d, NPT = %d isDownloadComplete [%d]", iFileSizeLastConvertedToTime, iLastNPTCalcInConvertSizeToTime, iDownloadComplete));
    PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCParserNode::playResumeNotification() - Auto Resume Triggered - FileSize = %d, NPT = %d", iFileSizeLastConvertedToTime, iLastNPTCalcInConvertSizeToTime));
    RunIfNotReady();
}

void PVMFEVRCFFParserNode::CompleteReset()
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::CompleteReset() Called"));
    /* stop and cleanup */
    ReleaseAllPorts();
    CleanupFileSource();
    SetState(EPVMFNodeIdle);
...
PVMFStatus PVMFEVRCFFParserNode::ThreadLogoff()
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::ThreadLogoff() Called"));
    if (iInterfaceState == EPVMFNodeIdle)
    {
        CleanupFileSource();
        iFileServer.Close();
    ...
    PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::ThreadLogoff() - Invalid State"));
    return PVMFErrInvalidState;
}

PVMFStatus PVMFEVRCFFParserNode::QueueMediaSample(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr)
{
    if (iAutoPaused == true)
    {
        aTrackInfoPtr->oQueueOutgoingMessages = false;
    ...
    if (aTrackInfoPtr->oQueueOutgoingMessages)
    {
        ...
        if (aTrackInfoPtr->oEOSReached == false)
        {
            ...
            if (status == PVMFErrBusy)
            {
                PVMF_EVRCPARSERNODE_LOGINFO((0, "PVMFEVRCParserNode::QueueMediaSample() RetrieveMediaSample - Mem Pools Busy"));
                aTrackInfoPtr->oQueueOutgoingMessages = false;
                if (iAutoPaused == true)
                {
            ...
            else if (status == PVMFSuccess)
            {
                if (aTrackInfoPtr->oEOSReached == false)
                {
                    ...
                    if (status != PVMFSuccess)
                    {
                        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::QueueMediaSample: Error - QueueOutgoingMsg failed"));
                        ReportErrorEvent(PVMFErrPortProcessing);
                    }
            ...
            {
                PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::QueueMediaSample() - Sample Retrieval Failed"));
...
PVMFStatus PVMFEVRCFFParserNode::RetrieveMediaSample(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr,
        PVMFSharedMediaDataPtr& aMediaDataOut)
{
    ...
    uint32 numsamples = NUM_EVRC_FRAMES;
    ...
    int32 retval = iEVRCParser->GetNextBundledAccessUnits(&numsamples, &gau);
    ...
    if (retval == evrcbitstreamObject::EVERYTHING_OK)
    {
        ...
        aTrackInfoPtr->iContinuousTimeStamp += numsamples * EVRC_SAMPLE_DURATION; // i.e 20ms
    ...
    else if (retval == evrcbitstreamObject::DATA_INSUFFICIENT)
	{
        ...
        if (iDownloadProgressInterface != NULL)
        {
            ...
            iDownloadProgressInterface->requestResumeNotification(aTrackInfoPtr->iContinuousTimeStamp,
                    iDownloadComplete);
            iAutoPaused = true;
            PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::RetrieveMediaSample() - Auto Pause Triggered - TS=%d", aTrackInfoPtr->iContinuousTimeStamp));
            PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCParserNode::RetrieveMediaSample() - Auto Pause Triggered - TS=%d", aTrackInfoPtr->iContinuousTimeStamp));
            return PVMFErrBusy;
        }
        else
        {
    ...
    else if (retval == evrcbitstreamObject::END_OF_FILE)
    {
        aTrackInfoPtr->oEOSReached = true;
    }
    else
    {
        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::RetrieveMediaSample() - Sample Retrieval Failed"));
        return PVMFFailure;
    }
...
void PVMFEVRCFFParserNode::PauseAllMediaRetrieval()
{
    PVEVRCFFNodeTrackPortInfo* trackInfoPtr = NULL;
    if (!GetTrackInfo(iOutPort, trackInfoPtr))
    {
        PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::PauseAllMediaRetrieval: Error - GetPortContainer failed"));
        return;
    }
...
PVMFStatus PVMFEVRCFFParserNode:: GenerateAndSendEOSCommand(PVEVRCFFNodeTrackPortInfo* aTrackInfoPtr)
{
    PVMF_EVRCPARSERNODE_LOGSTACKTRACE((0, "PVMFEVRCParserNode::GenerateAndSendEOSCommand Called"));
    if (aTrackInfoPtr->iPort->IsOutgoingQueueBusy() == true)
    {
        /* come back later */
        PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCParserNode::GenerateAndSendEOSCommand: Waiting - Output Queue Busy"));
        return PVMFErrBusy;
    }
    ...
    if ((aTrackInfoPtr->oEOSSent == false) && (aTrackInfoPtr->oEOSReached == true))
    {
        ...
        if (status != PVMFSuccess)
        {
            ...
            PVMF_EVRCPARSERNODE_LOGERROR((0, "PVMFEVRCParserNode::GenerateAndSendEOSCommand: Error Sending EOS"));
            return status;
        }
...
bool PVMFEVRCFFParserNode::RetrieveTrackData(PVEVRCFFNodeTrackPortInfo& aTrackPortInfo)
{
    // Create a data buffer from pool
    int errcode = 0;
    OsclSharedPtr<PVMFMediaDataImpl> mediaDataImplOut;
    ...
    // Temporary retrieve 32 frames since EVRC MDF needs 32 frames
    uint32 numsamples = NUM_EVRC_FRAMES;
    ...
    if (retval == evrcbitstreamObject::EVERYTHING_OK)
    {
        ...
        aTrackPortInfo.iContinuousTimeStamp += numsamples * EVRC_SAMPLE_DURATION; // i.e 20ms
    ...
    else if (retval == evrcbitstreamObject::READ_ERROR)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::RetrieveTrackData() EVRC Parser READ_ERROR"));
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::RetrieveTrackData() EVRC Parser End Of File!"));
        if (SendEndOfTrackCommand(aTrackPortInfo))
        {
            // EOS message sent so change state
            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_NOTICE, (0, "PVMFEVRCFFParserNode::RetrieveTrackData() Sending EOS message succeeded"));
            return false;
        }
        else
        {
            // EOS message could not be queued so keep in same state and try again later
            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_NOTICE, (0, "PVMFEVRCFFParserNode::RetrieveTrackData() Sending EOS message failed"));
            return true;
        }
    }
    else
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::RetrieveTrackData() EVRC Parser Unknown Error!"));
        PVUuid erruuid = PVMFFileFormatEventTypesUUID;
...
PVMFStatus PVMFEVRCFFParserNode::SendBeginOfMediaStreamCommand(PVEVRCFFNodeTrackPortInfo* aTrackPortInfoPtr)
{
    PVMFSharedMediaCmdPtr sharedMediaCmdPtr = PVMFMediaCmd::createMediaCmd();
    ...
    if (status != PVMFSuccess)
    {
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                        (0, "PVMFEVRCFFParserNode::SendBeginOfMediaStreamCommand: Outgoing queue busy. "));
        return status;
    }
    aTrackPortInfoPtr->iSendBOS = false;
    aTrackPortInfoPtr->oProcessOutgoingMessages = true;
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "PVMFEVRCFFParserNode::SendBeginOfMediaStreamCommand() BOS Sent StreamId %d ", iStreamID));
    return status;
}

PVMFCommandId
PVMFEVRCFFParserNode::GetLicense(PVMFSessionId aSessionId,
...
{
    PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCFFParserNode::GetLicense - Wide called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId,
                                        PVMF_EVRC_PARSER_NODE_GET_LICENSE_W,
                                        aContentName,
                                        aData,
                                        aDataSize,
                                        aTimeoutMsec,
                                ...
                                OsclAny* aContextData)
{
    ...
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommand::Construct(aSessionId,
                                        PVMF_EVRC_PARSER_NODE_GET_LICENSE,
                                        aContentName,
                                        aData,
                                        aDataSize,
                                        aTimeoutMsec,
...
PVMFStatus PVMFEVRCFFParserNode::DoGetLicense(PVMFEVRCFFNodeCommand& aCmd,
        bool aWideCharVersion)
{
    ...
    if (aWideCharVersion == true)
    {
        ...
        uint32 dataSize = 0;
        int32 timeoutMsec = 0;
        aCmd.PVMFEVRCFFNodeCommand::Parse(contentName,
                                         data,
                                         dataSize,
                                         timeoutMsec);
        iCPMGetLicenseCmdId =
...
void PVMFEVRCFFParserNode::CompleteGetLicense()
{
    CommandComplete(iCurrentCommand,
                    iCurrentCommand.front(),
                    PVMFSuccess);
...
PVMFCommandId
PVMFEVRCFFParserNode::CancelGetLicense(PVMFSessionId aSessionId, PVMFCommandId aCmdId, OsclAny* aContextData)
{
    PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCFFParserNode::CancelGetLicense - called"));
    PVMFEVRCFFNodeCommand cmd;
    cmd.PVMFEVRCFFNodeCommandBase::Construct(aSessionId, PVMF_EVRC_PARSER_NODE_CMD_CANCEL_GET_LICENSE, aCmdId, aContextData);
    return QueueCommandL(cmd);
}

PVMFStatus PVMFEVRCFFParserNode::DoCancelGetLicense(PVMFEVRCFFNodeCommand& aCmd)
{
    PVMF_EVRCPARSERNODE_LOGDATATRAFFIC((0, "PVMFEVRCFFParserNode::DoCancelGetLicense() Called"));
    ...
    else
    {
        ...
        if (cmd)
        {
            if (cmd->iCmd == PVMF_EVRC_PARSER_NODE_GET_LICENSE_W || cmd->iCmd == PVMF_EVRC_PARSER_NODE_GET_LICENSE)
            {
...
int32 PVMFEVRCFFParserNode::PushBackKeyVal(Oscl_Vector<PvmiKvp, OsclMemAllocator>*& aValueListPtr, PvmiKvp &aKeyVal)
{
    int32 leavecode = 0;
...
PVMFStatus PVMFEVRCFFParserNode::PushValueToList(Oscl_Vector<OSCL_HeapString<OsclMemAllocator>, OsclMemAllocator> &aRefMetaDataKeys, PVMFMetadataList *&aKeyListPtr, uint32 aLcv)
{
    int32 leavecode = 0;
    OSCL_TRY(leavecode, aKeyListPtr->push_back(aRefMetaDataKeys[aLcv]));
    OSCL_FIRST_CATCH_ANY(leavecode, PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMFEVRCFFParserNode::PushValueToList() Memory allocation failure when copying metadata key"));return PVMFErrNoMemory);
    return PVMFSuccess;
}
