 ...
 * -------------------------------------------------------------------
 */
#ifndef PVMF_AC3FFPARSER_NODE_H_INCLUDED
#define PVMF_AC3FFPARSER_NODE_H_INCLUDED
...
#ifndef PVMF_AC3FFPARSER_PORT_H_INCLUDED
...
#define PVMF_AC3_PARSER_NODE_NEW(auditCB,T,params,ptr)\
{\
	ptr = OSCL_NEW(T,params);\
}

#define PVMF_AC3_PARSER_NODE_DELETE(auditCB,T,ptr)\
{\
	OSCL_DELETE(ptr);\
}

#define PVMF_AC3_PARSER_NODE_TEMPLATED_DELETE(auditCB, T, Tsimple, ptr)\
{\
	OSCL_TEMPLATED_DELETE(ptr, T, Tsimple);\
}

#define PV_AC3_PARSER_NODE_ARRAY_NEW(auditCB, T, count, ptr)\
{\
	ptr = OSCL_ARRAY_NEW(T, count);\
}

#define PV_AC3_PARSER_NODE_ARRAY_DELETE(auditCB, ptr)\
{\
	OSCL_ARRAY_DELETE(ptr);\
}


typedef PVMFGenericNodeCommand<OsclMemAllocator> PVMFAC3FFNodeCommandBase;

/* Default vector reserve size */
#define PVMF_AC3_PARSER_NODE_VECTOR_RESERVE 10
/* Starting value for command IDs  */
#define PVMF_AC3_PARSER_NODE_COMMAND_ID_START 9000

enum TPVMFAC3ParserNodeCommand
{
    PVMF_AC3_PARSER_NODE_QUERYUUID,
    PVMF_AC3_PARSER_NODE_QUERYINTERFACE,
    PVMF_AC3_PARSER_NODE_REQUESTPORT,
    PVMF_AC3_PARSER_NODE_RELEASEPORT,
    PVMF_AC3_PARSER_NODE_INIT,
    PVMF_AC3_PARSER_NODE_PREPARE,
    PVMF_AC3_PARSER_NODE_START,
    PVMF_AC3_PARSER_NODE_STOP,
    PVMF_AC3_PARSER_NODE_FLUSH,
    PVMF_AC3_PARSER_NODE_PAUSE,
    PVMF_AC3_PARSER_NODE_RESET,
    PVMF_AC3_PARSER_NODE_CANCELALLCOMMANDS,
    PVMF_AC3_PARSER_NODE_CANCELCOMMAND,
    PVMF_AC3_PARSER_NODE_SET_DATASOURCE_POSITION,
    PVMF_AC3_PARSER_NODE_QUERY_DATASOURCE_POSITION,
    PVMF_AC3_PARSER_NODE_SET_DATASOURCE_RATE,
    PVMF_AC3_PARSER_NODE_GETNODEMETADATAKEYS,
    PVMF_AC3_PARSER_NODE_GETNODEMETADATAVALUES,
    PVMF_AC3_PARSER_NODE_SET_DATASOURCE_DIRECTION,
    PVMF_AC3_PARSER_NODE_GET_LICENSE_W,
    PVMF_AC3_PARSER_NODE_GET_LICENSE,
    PVMF_AC3_PARSER_NODE_CMD_CANCEL_GET_LICENSE,
    PVMF_AC3_PARSER_NODE_COMMAND_LAST
};

#define PVMF_AC3_PARSER_NODE_MAX_CPM_METADATA_KEYS 256

class PVMFAC3FFNodeCommand : public PVMFAC3FFNodeCommandBase
{
    public:
        ...
        {
            PVMFAC3FFNodeCommandBase::Construct(s, cmd, aContext);
        ...
        void Destroy()
        {
            PVMFGenericNodeCommand<PVMFAC3ParserNodeAllocator>::Destroy();
            switch (iCmd)
            {
                case PVMF_AC3_PARSER_NODE_GETNODEMETADATAKEYS:
                    if (iParam4)
                    {
                        ...
                        Oscl_TAlloc<OSCL_HeapString<PVMFAC3ParserNodeAllocator>, PVMFAC3ParserNodeAllocator> str;
        ...
        void Copy(const PVMFGenericNodeCommand<PVMFAC3ParserNodeAllocator>& aCmd)
        {
            PVMFGenericNodeCommand<PVMFAC3ParserNodeAllocator>::Copy(aCmd);
            switch (aCmd.iCmd)
            {
                case PVMF_AC3_PARSER_NODE_GETNODEMETADATAKEYS:
                    if (aCmd.iParam4)
                    {
                        ...
                        OSCL_HeapString<PVMFAC3ParserNodeAllocator>* aStr =
                            (OSCL_HeapString<PVMFAC3ParserNodeAllocator>*)aCmd.iParam4;
                        Oscl_TAlloc<OSCL_HeapString<PVMFAC3ParserNodeAllocator>, PVMFAC3ParserNodeAllocator> str;
...
typedef PVMFNodeCommandQueue<PVMFAC3FFNodeCommand, OsclMemAllocator> PVMFAC3FFNodeCmdQ;
...
typedef OsclMemAllocator PVMFAC3FFNodeAllocator;

/**
* The node class
*/
//typedef PVMFNodeCommandQueue<PVMFAC3FFNodeCommand, PVMFAC3ParserNodeAllocator> PVMFAC3ParserNodeCmdQueue;
class CAC3FileParser;
class PVMFAC3FFParserOutPort;
class PVLogger;

class PVMFAC3FFParserNode :  public OsclTimerObject
...
{
    public:
        PVMFAC3FFParserNode(int32 aPriority = OsclActiveObject::EPriorityNominal);
        ~PVMFAC3FFParserNode();
        ...
        PVMFCommandId QueryUUID(PVMFSessionId, const PvmfMimeString& aMimeType,
                                Oscl_Vector<PVUuid, PVMFAC3FFNodeAllocator>& aUuids,
    ...
    private:
        ...
        void Run();
        //Command processing
        PVMFCommandId QueueCommandL(PVMFAC3FFNodeCommand& aCmd);

        void MoveCmdToCurrentQueue(PVMFAC3FFNodeCommand& aCmd);
        void MoveCmdToCancelQueue(PVMFAC3FFNodeCommand& aCmd);
        void ProcessCommand();
        void CommandComplete(PVMFAC3FFNodeCmdQ& aCmdQueue,
        ...
        void CommandComplete(PVMFAC3FFNodeCmdQ&, PVMFAC3FFNodeCommand&, PVMFStatus, PVInterface*, OsclAny* aData);
        bool FlushPending();
        //Command handlers.
        void DoQueryUuid(PVMFAC3FFNodeCommand&);
        void DoQueryInterface(PVMFAC3FFNodeCommand&);

        PVMFStatus DoInit(PVMFAC3FFNodeCommand&);
        void CompleteInit();

        void DoPrepare(PVMFAC3FFNodeCommand&);
        void DoStart(PVMFAC3FFNodeCommand&);
        void DoStop(PVMFAC3FFNodeCommand&);
        void DoPause(PVMFAC3FFNodeCommand&);
        void DoFlush(PVMFAC3FFNodeCommand&);

        void DoCancelAllCommands(PVMFAC3FFNodeCommand&);
        void DoCancelCommand(PVMFAC3FFNodeCommand&);

        void DoRequestPort(PVMFAC3FFNodeCommand&, PVMFPortInterface*&);
        void DoReleasePort(PVMFAC3FFNodeCommand&);

        void DoReset(PVMFAC3FFNodeCommand&);
        void  CompleteReset();

        // For metadata extention interface
        PVMFStatus DoGetMetadataKeys(PVMFAC3FFNodeCommand& aCmd);
        PVMFStatus DoGetMetadataValues(PVMFAC3FFNodeCommand& aCmd);
        ...
        PVMFStatus CompleteGetMetadataKeys(PVMFAC3FFNodeCommand& aCmd);

        void PushToAvailableMetadataKeysList(const char* aKeystr, char* aOptionalParam = NULL);
        Oscl_Vector<OSCL_HeapString<PVMFAC3ParserNodeAllocator>, PVMFAC3ParserNodeAllocator> iAvailableMetadataKeys;
        Oscl_Vector<OSCL_HeapString<PVMFAC3ParserNodeAllocator>, PVMFAC3ParserNodeAllocator> iCPMMetadataKeys;
        uint32 iAC3ParserNodeMetadataValueCount;

        // For data source position extension interface
        PVMFStatus DoSetDataSourcePosition(PVMFAC3FFNodeCommand& aCmd);
        PVMFStatus DoQueryDataSourcePosition(PVMFAC3FFNodeCommand& aCmd);
        PVMFStatus DoSetDataSourceRate(PVMFAC3FFNodeCommand& aCmd);

        /* For data source direction extension interface */
        void DoSetDataSourceDirection(PVMFAC3FFNodeCommand& aCmd);
        ...
        bool RetrieveTrackData(PVAC3FFNodeTrackPortInfo& aTrackPortInfo);
        PVMFStatus RetrieveTrackData(PVAC3FFNodeTrackPortInfo& aTrackPortInfo, PVMFSharedMediaDataPtr& aMediaDataOut);
        bool SendTrackData(PVAC3FFNodeTrackPortInfo& aTrackPortInfo);
        bool SendEndOfTrackCommand(PVAC3FFNodeTrackPortInfo& aTrackPortInfo);
        PVMFStatus SendBeginOfMediaStreamCommand(PVAC3FFNodeTrackPortInfo* aTrackPortInfo);
        bool CheckAvailabilityForSendingNewTrackData(PVAC3FFNodeTrackPortInfo& aTrackPortInfo);
        void ResetAllTracks();
    ...
    private: // private member variables
        ...
        PVMFAC3FFNodeCmdQ iInputCommands;
        PVMFAC3FFNodeCmdQ iCurrentCommand;
        PVMFAC3FFNodeCmdQ iCancelCommand;

        PVMFAC3FFParserOutPort* iOutPort;
        friend class PVMFAC3FFParserOutPort;
        void ProcessOutgoingMsg();

        /* Port processing */
        PVMFPortVector<PVMFAC3FFParserOutPort, PVMFAC3ParserNodeAllocator> iPortVector;
        bool ProcessPortActivity(PVAC3FFNodeTrackPortInfo*);
        PVMFStatus RetrieveMediaSample(PVAC3FFNodeTrackPortInfo* aTrackInfoPtr,
                                       PVMFSharedMediaDataPtr& aSharedPtr);
        PVMFStatus QueueMediaSample(PVAC3FFNodeTrackPortInfo* aTrackInfoPtr);
        PVMFStatus ProcessOutgoingMsg(PVAC3FFNodeTrackPortInfo* aTrackInfoPtr);
        bool GetTrackInfo(PVMFPortInterface* aPort,
                          PVAC3FFNodeTrackPortInfo*& aTrackInfoPtr);
        bool GetTrackInfo(int32 aTrackID,
                          PVAC3FFNodeTrackPortInfo*& aTrackInfoPtr);
        PVMFStatus GenerateAndSendEOSCommand(PVAC3FFNodeTrackPortInfo* aTrackInfoPtr);
        bool CheckForPortRescheduling();
        bool CheckForPortActivityQueues();

        /* Progressive download related */
        PVMFStatus CheckForAC3HeaderAvailability();
        void PauseAllMediaRetrieval();
        uint64 iAC3HeaderSize;
        bool iAutoPaused;
        ...
        CAC3FileParser* iAC3Parser;
        TPVAc3FileInfo iAC3FileInfo;
        ...
        Oscl_Vector<PVAC3FFNodeTrackPortInfo, OsclMemAllocator> iSelectedTrackList;
        ...
        PVMFStatus DoGetLicense(PVMFAC3FFNodeCommand& aCmd,
                                bool aWideCharVersion = false);
        PVMFStatus DoCancelGetLicense(PVMFAC3FFNodeCommand& aCmd);
        void CompleteGetLicense();
        void Assert(bool);
        PVMFStatus ParseAC3File();
        uint32 iCountToClaculateRDATimeInterval;
};

#endif // PVMF_AC3FFPARSER_NODE_H_INCLUDED
