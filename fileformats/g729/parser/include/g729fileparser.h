...
/**
 *  @file g729fileparser.h
 *  @brief This file defines the raw GSM-G729 file parser.
 */

#ifndef G729FILEPARSER_H_INCLUDED
#define G729FILEPARSER_H_INCLUDED
...
#define PVMF_G729PARSER_LOGDIAGNOSTICS(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG,iDiagnosticLogger,PVLOGMSG_INFO,m);
#define PVMF_G729PARSER_LOGERROR(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iLogger,PVLOGMSG_ERR,m);
#define PVMF_G729PARSER_LOGDEBUG(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iLogger,PVLOGMSG_DEBUG,m);
...
#define G729_MAX_NUM_PACKED_INPUT_BYTES  160 /* Max number of packed input bytes     */
...
#define TIME_STAMP_PER_FRAME        10 //80 /* Time stamp value per frame of G729 */
...
/**
 *  @brief The g729bitstreamObject Class is the class used by the G729 parser to
 *  manipulate the bitstream read from the file.
 */
class g729bitstreamObject
{
    public:
        enum
        {
            MAIN_BUFF_SIZE = 8192,
            SECOND_BUFF_SIZE = 160,//assuming max frame size of G729 as 160
        ...
        * @returns None
        */
        g729bitstreamObject(PVLogger *aLogger,PVFile* pFile = NULL)
        {
            oscl_memset(this, 0, sizeof(g729bitstreamObject));
            iLogger = aLogger;
            init(pFile);
            iBuffer = OSCL_ARRAY_NEW(uint8, g729bitstreamObject::MAIN_BUFF_SIZE + g729bitstreamObject::SECOND_BUFF_SIZE);
            if (iBuffer)
            {
                iStatus = true;
            }
    ...
    private:
        ...
        */
        inline void init(PVFile* pFile = NULL)
        {
            ...
            iBytesProcessed = 0;
            ipG729File = pFile;
            iActual_size = iMax_size = g729bitstreamObject::MAIN_BUFF_SIZE;
            ...
            if (ipG729File)
            {
                ipG729File->Seek(0, Oscl_File::SEEKSET);
            }
        }
        ...
        */
        ...
        /**
        * @brief Parses the G729 bitstream header: "$!G729"
        *
        * @param None
        * @returns Result of operation: EVERYTHING_OK, READ_FAILED, etc.
        */
        int32 parseG729Header();
    ...
    private:
        ...
        int32 iFileSize;        // file size of the ipG729File
        ...
        uint8 *iBuffer;
        PVFile* ipG729File; // bitstream file
        bool iStatus;
        PVLogger *iLogger;
};

/**
 *  @brief The CG729FileParser Class is the class that will construct and maintain
 *  all the necessary data structures to be able to render a valid G729 file
 *  to disk.
 *
 *  This class supports the following G729 file format specs: IF2, WMF, ETS
 */
...
class CG729FileParser
{
    public:
        ...
        * @returns None
        */
        OSCL_IMPORT_REF  CG729FileParser();
        ...
        */
        OSCL_IMPORT_REF bool InitG729File(OSCL_wString& aClip, bool aInitParsingEnable = true, Oscl_FileServer* aFileSession = NULL, PVMFCPMPluginAccessInterfaceFactory*aCPM = NULL, OsclFileHandle*aHandle = NULL, uint32 countToClaculateRDATimeInterval = 1);
        ...
        */
        OSCL_IMPORT_REF bool RetrieveFileInfo(TPVG729FileInfo& aInfo);
        ...
        */
        ...
        /**
        * @brief Attempts to read in the number of G729 frames specified by aNumSamples.
        ...
        */
        ...
        /**
        * @brief Returns the frame type of the current G729 frame
        *
    ...
    private:
        ...
        uint8 iG729FrameBuffer[G729_MAX_NUM_PACKED_INPUT_BYTES];
        uint8 iG729FrameHeaderBuffer[MAX_NUM_FRAMES_PER_BUFF];
...
#endif //G729FILEPARSER_H_INCLUDED
